...
def parse_txt_file(content):
    """Parse the TXT file format and extract questions with better accuracy"""
    questions = []
    
    # Split by question numbers (1., 2., 3., etc.)
    # Use a more robust regex to split questions
    question_blocks = re.split(r'(?=\d+\.\s)', content.strip())
    ...
    for block_idx, block in enumerate(question_blocks):
        ...
        if len(lines) < 3:  # Minimum: question, one option, and answer
            continue
        ...
        # Extract question (skip the number at beginning)
        # Remove question number if present (e.g., "1. ", "2. ", etc.)
        first_line = lines[0]
        ...
        # Continue adding question lines until we hit an option
        while (current_line < len(lines) and 
               not re.match(r'^[a-e]\)\s*', lines[current_line], re.IGNORECASE) and
               not lines[current_line].startswith('Correct') and
               not lines[current_line].startswith('ex:')):
        ...
        while (current_line < len(lines) and 
               re.match(r'^[a-e]\)\s*', lines[current_line], re.IGNORECASE)):
            # Start of a new option
            if option_lines and option_count < 5:
                question[f"option_{option_count}"] = '<br>'.join(option_lines)
                option_count += 1
            
            # Get the option letter and text
            option_match = re.match(r'^([a-e])\)\s*(.*)$', lines[current_line], re.IGNORECASE)
            option_letter = option_match.group(1).lower() if option_match else ''
            option_text = option_match.group(2) if option_match else lines[current_line][2:].strip()
            
            option_lines = [option_text]
            current_line += 1
            
            # Check if next line is continuation of this option (not a new option and not answer/explanation)
            while (current_line < len(lines) and 
                   not re.match(r'^[a-e]\)\s*', lines[current_line], re.IGNORECASE) and
                   not lines[current_line].startswith('Correct') and
                   not lines[current_line].startswith('ex:')):
                option_lines.append(lines[current_line])
                current_line += 1
        ...
        while current_line < len(lines) and lines[current_line].startswith('Correct'):
            answer_line = lines[current_line]
            # Handle different formats: "Correct option:-a", "Correct:-a", "Correct option: a", etc.
            answer_match = re.search(r'[Cc]orrect[^:]*[:-]\s*([a-eA-E])', answer_line)
            if answer_match:
        ...
        while current_line < len(lines):
            ...
            if line.startswith('ex:') or line.startswith('Ex:') or line.startswith('EX:'):
                # Remove "ex:" prefix
                explanation = re.sub(r'^[Ee][Xx]:\s*', '', line)
                solution_lines.append(explanation)
            elif solution_lines and not line.startswith('Correct') and not re.match(r'^[a-e]\)', line):
                # Continuation of explanation without "ex:" prefix
                solution_lines.append(line)
            current_line += 1
        
        question["solution_text"] = '<br>'.join(solution_lines)
        
        # Validate we have at least question, 2 options, and answer
        if (question["question"] and 
            question["option_1"] and 
            question["option_2"] and 
            question["answer"]):
            questions.append(question)
        else:
            logger.warning(f"Question {block_idx + 1} missing required fields, skipping")
    
    # Log parsing results
    logger.info(f"Successfully parsed {len(questions)} questions from file")
    
    return questions

def validate_parsed_questions(questions):
    """Validate that all questions have been parsed correctly"""
    validation_errors = []
    ...
    for i, q in enumerate(questions):
        ...
        option_count = sum(1 for key in ["option_1", "option_2", "option_3", "option_4", "option_5"] if q.get(key))
        if option_count < 2:
            errors.append(f"Only {option_count} options found (minimum 2 required)")
        
        # Check answer is valid (1-5)
        if q.get("answer") and q["answer"] not in ['1', '2', '3', '4', '5']:
            errors.append(f"Invalid answer: {q.get('answer')}")
        
        # Check answer corresponds to an existing option
        answer_num = int(q.get("answer", 0))
        if answer_num > 0 and not q.get(f"option_{answer_num}"):
            errors.append(f"Answer points to option_{answer_num} which doesn't exist")
...
#palette .qbtn.answered{{background:var(--accent);color:#fff;border:none}}
#palette .qbtn.current{{background:rgba(46,196,182,0.12);border:2px solid var(--accent)}}
...
@media(max-width:540px){{
  ...
  .fbar-inner{{gap:8px;padding:8px}}
  .fbar button{{padding:8px 10px;font-size:13px}}
}}
...
<div id="ssc-join-overlay" aria-hidden="true">
  <div id="ssc-join-card">
    <button id="ssc-join-close" aria-label="Close">✕</button>
...
const EXPECTED_HASH = 197247123;  // 👈 PASTE YOUR HASH HERE

if (
  _hx(banner.textContent.trim() + "|" + joinBtn.href)
  === EXPECTED_HASH
) {{
  __QUIZ_OK = true;
}} else {{
...
function checkNameBeforeSubmit() {{
  const nameInput = document.getElementById("studentName");

  if (!nameInput || !nameInput.value.trim()) {{
    alert("Please enter your name to submit quiz");
    nameInput?.focus();   // cursor returns to name field
...
function init(){{
  ...
  if (resultSaved) {{
    ...
    if (data.submitted && data.resultHTML) {{
      ...
      document.getElementById("studentName")?.style.setProperty("display", "none");
...
function highlightPalette(){{
  ...
  Array.from(pal.children).forEach((child, idx) => {{
    ...
    child.classList.remove("answered","current");
    const qid = QUESTIONS[idx].id ?? idx;
    if(idx === current) child.classList.add("current");
    else if(answers[qid]) child.classList.add("answered");
  }});
  const summary = el("palette-summary");
  if(summary) summary.textContent = `Total: ${{total}} | Attempted: ${{attempted}} | Not Attempted: ${{notAttempted}}`;
}}
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle TXT file upload with validation"""
    update_activity()
    ...
    try:
        ...
        if content is None:
            await update.message.reply_text("❌ Could not decode file. Please use UTF-8 or ASCII encoding.")
            return GETTING_FILE
        ...
        if not questions:
            await update.message.reply_text(
                "❌ Could not parse any questions from the file. Please check the format.\n\n"
                "Expected format:\n"
                "1. Question text\n"
                "a) Option 1\n"
                "b) Option 2\n"
        ...
        validation_errors = validate_parsed_questions(questions)
        if validation_errors:
            error_msg = f"⚠️ Found {len(validation_errors)} issues:\n\n"
            for i, error in enumerate(validation_errors[:5]):  # Show first 5 errors
                error_msg += f"{i+1}. {error}\n"
            
            if len(validation_errors) > 5:
                error_msg += f"\n... and {len(validation_errors) - 5} more issues"
            
            error_msg += "\n\nWould you like to continue anyway?"
            
            keyboard = [
                [InlineKeyboardButton("✅ Continue", callback_data="continue_parse")],
                [InlineKeyboardButton("❌ Cancel", callback_data="cancel_parse")]
            ]
            ...
            context.user_data["questions_temp"] = questions
            context.user_data["file_name"] = document.file_name
            return GETTING_FILE
        
        # Store in context if no validation errors
...
async def handle_parse_validation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle validation response"""
    query = update.callback_query
    await query.answer()
    
    if query.data == "continue_parse":
        # Use the temporarily stored questions
        context.user_data["questions"] = context.user_data.get("questions_temp", [])
        del context.user_data["questions_temp"]
        
        await query.edit_message_text(f"✅ Continuing with {len(context.user_data['questions'])} questions.\n\nNow enter the quiz name:")
        return GETTING_QUIZ_NAME
    else:  # cancel_parse
        await query.edit_message_text("❌ Parse cancelled. Please send a corrected file.")
...
*File Format:*
Your TXT file should have this format:
...
• Improved file parsing with validation
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            GETTING_FILE: [
                ...
                CallbackQueryHandler(handle_parse_validation, pattern="^(continue_parse|cancel_parse)$"),
